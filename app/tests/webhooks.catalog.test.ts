...
describe("catalog webhook route", () => {
  ...
  it("accepts authenticated collection topics and successful duplicate results", async () => {
    const collection = {...authenticated, topic: "COLLECTIONS_UPDATE" as const};
    const ingest = vi.fn().mockResolvedValue({record: {}, duplicate: true});
    const action = createCatalogWebhookAction(vi.fn().mockResolvedValue(collection), ingest);

    const response = await action(actionArgs());

    expect(response.status).toBe(200);
    expect(ingest).toHaveBeenCalledWith(collection);
  });
